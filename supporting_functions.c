...
void vPrintFourStrings(const char* pcString1, const char* pcString2, const char* pcString3, const char* pcString4)
{
	/* Print the string, using a critical section as a crude method of mutual
	exclusion. */
	vTaskSuspendAll();
	{
		printf("%s  %s %s %s\r\n", pcString1, pcString2, pcString3, pcString4);
	}
	xTaskResumeAll();
...
void vPrintThreeNumbers(double ulValue1, double ulValue2, double ulValue3)
{
	/* Print the string, using a critical section as a crude method of mutual
	exclusion. */
	taskENTER_CRITICAL();
	{
		printf("%lu    %f     %f     %f\r\n", xTaskGetTickCount(), ulValue1, ulValue2, ulValue3);
		fflush(stdout);
	}
	taskEXIT_CRITICAL();
